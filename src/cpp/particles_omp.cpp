...
/*
================================================================================
Particle System Solver - OpenMP C++17 Port
================================================================================

Parallel (OpenMP) port of the serial C++17 baseline (particles.cpp).

The numerical model is IDENTICAL to the baseline. Only the heavy regions are
parallelized:
  - computeForces:                 #pragma omp parallel for schedule(static) over i.
                                   The INNER j-loop order (j = 0..N-1) is UNCHANGED,
                                   so each fx[i]/fy[i] accumulates in the same order
                                   as the serial code -> positions/velocities stay
                                   (near) bitwise-identical.
  - integrateVV (both per-i loops):#pragma omp parallel for.
  - computeGeneratingField:        #pragma omp parallel for collapse(2).
  - computeValidationQuantities:   reduction(+:...) for the linear sums and for the
                                   O(N^2) potential_like double loop (small
                                   reduction-order diffs are tolerable).

Particle generation order is kept serial (it is part of the contract and cheap).

HDF5 support is optional at compile time (USE_HDF5).

Without HDF5 (Apple clang + libomp):

  clang++ -O3 -std=c++17 -Wall -Wextra -Wpedantic -Xpreprocessor -fopenmp \
      particles_omp.cpp -o particles_omp -lomp

Without HDF5 (GCC):

  g++ -O3 -std=c++17 -Wall -Wextra -Wpedantic -fopenmp \
      particles_omp.cpp -o particles_omp

With HDF5 (GCC):

  g++ -O3 -std=c++17 -Wall -Wextra -Wpedantic -fopenmp -DUSE_HDF5 \
      particles_omp.cpp -o particles_omp -lhdf5_cpp -lhdf5

IMPORTANT for numerics: do NOT enable fast-math / FMA contraction; it would change
the per-element results and break tight validation.

Command line (identical to baseline):

  ./particles_omp [inputFile] [h5File|none|--no-hdf5] [outputEvery]
...
// Embarrassingly parallel: each (i, j) cell is independent.
// ============================================================
...
#pragma omp parallel for collapse(2) schedule(static)
    for (std::size_t j = 0; j < ny; ++j) {
        for (std::size_t i = 0; i < nx; ++i) {
            ...
            g.values[idx2D(i, j, nx)] = static_cast<unsigned long long>(iter);
        }
    }
}
...
// ============================================================
// PARTICLE GENERATION
// Kept serial: the selection order fixes the particle index used by /pos and
// /vel, so it is part of the contract. This step is cheap.
// ============================================================
...
// ============================================================
// Parallelized over the OUTER index i only (schedule(static)). The INNER j-loop
// order is preserved exactly (j = 0..N-1), so each fx[i]/fy[i] accumulates in
// the same order as the serial baseline. No pair symmetry / no write races.
// ============================================================
...
// Both per-i loops are parallelized; each i writes only its own elements.
// ============================================================
...
// so this function should not affect benchmark timing. Kept serial (identical
// to the baseline) to preserve scatter-accumulation behavior exactly.
// ============================================================
...
// Linear sums use reduction(+:...). The O(N^2) potential_like double loop uses
// reduction(+:potential); small reduction-order differences are tolerable and
// do not affect the tight position/velocity checks.
// ============================================================
...
ValidationQuantities computeValidationQuantities(const Particles& P) {
    ...
    const double* const x  = P.x.data();
    const double* const y  = P.y.data();
    const double* const vx = P.vx.data();
    const double* const vy = P.vy.data();
...
#pragma omp parallel for schedule(static) \
    reduction(+:sum_x, sum_y, sum_vx, sum_vy, weighted_sum_x, weighted_sum_y, \
...
#pragma omp parallel for schedule(static) reduction(+:potential)
    for (std::size_t i = 0; i < N; ++i) {
        for (std::size_t j = i + 1; j < N; ++j) {
            ...
            potential += kForce * w[i] * w[j] / std::sqrt(r2);
        }
    }
