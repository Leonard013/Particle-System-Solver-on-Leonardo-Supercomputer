...
/*
================================================================================
Particle System Solver - MPI port (distributed memory, C++17)
================================================================================

MPI parallelization of the serial baseline. Design:

  * Block decomposition of the OUTER particle index: rank r owns the
    contiguous slice [i0_r, i1_r). The inner force-sum order (j = 0..N-1)
    is preserved, so per-particle arithmetic is identical to serial and
    results are expected to be bitwise-identical (same invariant as the
    OpenMP port).
  * Initialization (Mandelbrot field + particle generation) is executed
    redundantly by every rank: it is deterministic and cheap (~0.15 s on
    the official input), and redundancy guarantees a bit-identical initial
    state with zero communication.
  * Per velocity-Verlet step: each rank drifts its slice, positions are
    synchronized with MPI_Allgatherv(MPI_IN_PLACE, ...) -- the Allgather
    pattern for all-pairs problems -- then each rank computes forces for
    its slice against ALL N particles.
  * Velocities stay slice-local during dynamics; they are allgathered only
    on HDF5 output steps and once after the loop for validation.
  * "Pure dynamics time" includes the per-step communication (the honest
    parallel cost). Ranks are aligned with MPI_Barrier before the loop and
    the timing is reduced with MPI_MAX across ranks.
  * Rank 0 performs all prints and HDF5 I/O (master-worker convention).
...
// MPI DECOMPOSITION
// ============================================================
// Block distribution of the particle index space over ranks, plus the
// counts/displacements needed by MPI_Allgatherv.

struct MpiSlice {
    int rank{0};
    int size{1};
    std::size_t i0{0};              // first owned particle
    std::size_t i1{0};              // one past the last owned particle
    std::vector<int> counts;        // owned count per rank
    std::vector<int> displs;        // slice offset per rank
    ...
    void init(std::size_t N) {
        ...
        i0 = static_cast<std::size_t>(displs[static_cast<std::size_t>(rank)]);
        i1 = i0 + static_cast<std::size_t>(counts[static_cast<std::size_t>(rank)]);
    }

    // Synchronize a full-length array: every rank contributes the slice it
    // owns, in place. Bits are moved verbatim, so synchronization cannot
    // perturb the numerics.
    void allgather(std::vector<double>& a) const {
        MPI_Allgatherv(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL,
                       a.data(), counts.data(), displs.data(), MPI_DOUBLE,
                       MPI_COMM_WORLD);
    }
};
...
// MPI variant of the serial computeForces: computes forces only for the
// owned slice [i0, i1). The inner loop still sweeps ALL particles in the
// serial order j = 0..N-1, so each owned particle's force sum is
// bitwise-identical to the serial result.
void computeForcesSlice(const Particles& P, std::size_t i0, std::size_t i1, double* fx, double* fy) {
    const std::size_t N = P.n;

    if (!fx || !fy) {
        throw std::runtime_error("computeForcesSlice: null output pointer");
    }
...
void integrateVV(
    ...
    // Half kick + drift: owned slice only.
    for (std::size_t i = S.i0; i < S.i1; ++i) {
        assert(P.w[i] > 0.0);
    ...
    // Every rank needs the drifted positions of ALL particles for the
    // all-pairs force sweep (Allgather pattern).
    S.allgather(P.x);
    S.allgather(P.y);

    computeForcesSlice(P, S.i0, S.i1, fx_new.data(), fy_new.data());

    // Second half kick: owned slice only.
    for (std::size_t i = S.i0; i < S.i1; ++i) {
        assert(P.w[i] > 0.0);
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        computeForcesSlice(P, S.i0, S.i1, fx.data(), fy.data());
        ...
        auto writeOutputFrame = [&](std::size_t step) {
            ...
            // Collective on ALL ranks: gather the slice-local velocities so
            // the frame contains the full state. Positions are already fully
            // synchronized by the in-step allgather (or by initialization).
            S.allgather(P.vx);
            S.allgather(P.vy);

            if (!h5) {
                // Non-root ranks only participate in the collective; they
                // track the dedup state so every rank makes the same
                // allgather decisions.
        ...
        for (std::size_t step = 0; step < cfg.maxSteps; ++step) {
            if (writeHdf5 && shouldWriteStep(step, cfg.maxSteps, cfg.outputEvery)) {
                writeOutputFrame(step);
            }

            const double dynStart = MPI_Wtime();
            integrateVV(P, S, fx, fy, fx_new, fy_new, cfg.dt);
            const double dynStop = MPI_Wtime();

            pureDynamicsSeconds += dynStop - dynStart;
        }
        ...
        // 8. Validation
        // ----------------------------------------------------
        // Parallel dynamics time = the slowest rank's time (MPI_MAX).
        {
            double localDyn = pureDynamicsSeconds;
            MPI_Reduce(root ? MPI_IN_PLACE : &localDyn, &localDyn, 1,
                       MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
            pureDynamicsSeconds = localDyn;
        }

        // Full velocities everywhere for validation (positions already synced).
        S.allgather(P.vx);
        S.allgather(P.vy);

        const auto validationStart = std::chrono::steady_clock::now();
        ValidationQuantities validation{};
        if (root) {
            // Serial reduction over the full state on rank 0: identical
            // summation order to the serial baseline, so the printed
            // 17-digit block is expected to match serial bitwise.
            validation = computeValidationQuantities(P);
        }
